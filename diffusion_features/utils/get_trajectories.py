...
def increase_trajectory_resolution(trajectory):
...
def calculate_conditioning(trajectory, lava_pos, key_pos):
    # calculate min distance to lava
    lava_dist = np.linalg.norm(trajectory - lava_pos, axis=-1).min()
    # calculate min distance to key
    key_dist = np.linalg.norm(trajectory - key_pos, axis=-1).min()
    # calculate trajectory length pre-edge-padding
    traj_len = trajectory.shape[0]
    
    conditioning = [trajectory[0][0], trajectory[0][1], trajectory[-1][0], trajectory[-1][1], 
                    lava_pos[0], lava_pos[1], key_pos[0], key_pos[1],
                    traj_len, lava_dist, key_dist]
    return conditioning


def get_trajectories(data_path: str):
    ...
    for file in files:
        ...
        with open(absolute_path_, 'r') as f:
            ...
            lava_pos = np.array(data['condition'][1].split(' ')).astype(int)
            key_pos = np.array(data['condition'][2].split(' ')).astype(int)
            conditioning = np.array(calculate_conditioning(trajectory, lava_pos, key_pos))
            trajectory = increase_trajectory_resolution(trajectory)
            # trajectory = np.array(data['positions'])
            # remove the adjacent position duplicates
            # trajectory = trajectory[~(np.roll(trajectory, 1, axis=0) == trajectory).all(axis=1)]
            trajectories[file] = trajectory
            conditionings[file] = conditioning
    ...
    conditionings_tensor = torch.tensor([conditioning for conditioning in conditionings.values()])
    # print(trajectories_tensor.shape)
    return trajectories_tensor, conditionings_tensor
